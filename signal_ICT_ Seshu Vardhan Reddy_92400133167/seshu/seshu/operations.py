# -*- coding: utf-8 -*-
"""
Created on Sat Sep  6 10:34:43 2025
...
def time_shift(signal, k):
    ...
    Positive k shifts the signal to the right, negative k shifts the signal to the left.
    """
    return np.roll(signal, k)

def time_scale(signal, k):
    """
    Scales the time axis of the signal by a factor of k.
    If k > 1, the signal is stretched; if 0 < k < 1, the signal is compressed.
    """
    n = len(signal)
...
def signal_addition(signal1, signal2):
    """
    Performs element-wise addition of two signals.
    """
...
def signal_multiplication(signal1, signal2):
    """
    Performs element-wise multiplication of two signals.
    """
    if len(signal1) != len(signal2):
        raise ValueError("Signals must have the same length")
    return signal1 * signal2
